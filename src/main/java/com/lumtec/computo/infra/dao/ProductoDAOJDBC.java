...
public class ProductoDAOJDBC implements ProductoDAO {
    ...
    public ProductoDAOJDBC(EntityManager em) {
        this.em = em;
    }
    ...
    public List<Producto> getProductosInventario() {
        String jpql = "SELECT NEW com.lumtec.computo.infra.model.Producto(P.nombre, P.marca, P.modelo, P.cantidad, P.precioVenta) FROM Producto AS P";
    ...
    public void actualizar(Producto producto) {
        ...
        try (con) {
            pps = con.prepareStatement("UPDATE inventario SET NOMBRE=?, MARCA=?, MODELO=?, COLOR=?, DESCRIPCION=?,"
                    ...
                    + "PORCENTAJE_REINVERSION = ?, PROVEDOR = ?, GARANTIA= ? WHERE ID = " + producto.getId() + "");
            try (pps) {
    ...
    public void insertarPrecio(Producto prod) {
        ...
        try {
            con = this.conexionTransaccional != null ? this.conexionTransaccional : Conexion.getConnection();
            pps = con.prepareStatement("UPDATE inventario SET PRECIO_COMPRA = ?, PRECIO_VENTA = ?, GANANCIA = ?, REINVERSION = ?, PORCENTAJE_GANANCIA = ?, PORCENTAJE_REINVERSION = ?  WHERE id_producto = " + prod.getId() + ";");
